...
- **映射**：`0-EMPTY`, `1-BLACK`, `2-WHITE`。
- **意义**：规避频繁调用 `Board.get()` 的开销，支持秒级的 `apply/undo` 操作。
...
- **搜索**：深度优先的 Alpha-Beta 搜索。
- **生成**：
  - **一级优先**：强制响应威胁（Win/Block）。
  - **二级优先**：启发式高分候选点组合（Top 12 候选点）。
...
- [x] **V1 达成(着法生成)**：能够防御 2 个以内的威胁，不产生异常。
  - [x] (a) 胜着发现：在 generateMoves 中，优先级最高的是检测己方连5或连4（能凑成6）的必胜点。
  - [x] (b) 威胁计算与防守：实现了 scanThreats 方法，能够快速识别对方的“冲4”和“活3”等威胁。当威胁数量可控时，AI会优先生成防守着法。
  - [x] (c) 智能选点：在 getCandidates
    中实现了启发式评分，优先选择靠近棋盘中心（Centrality）且周围有棋子（Adjacency）的位置。
  - [x] (d) 异常处理：加入了边界检查和空着法兜底逻辑，确保不会抛出异常。
- [x] **V2 达成(博弈树搜索)**：具备快速盘面评估与有效的 AB 剪枝。
  - [x] (a) 快速评估：实现了 evaluate() 函数，针对全盘所有线上的棋型（如活三、死四）进行加权打分。
  - [x] (b) 有效路与剪枝：实现了标准的 Alpha-Beta
    剪枝算法（alphaBetaSearch），并限制搜索深度。通过只搜索高分候选点（有效路），大幅减少了计算量。
